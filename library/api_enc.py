...
class ApiEnc:
    ...
    """
    # 项目信息
    def __init__(self, key=settings.APP_KEY, charset=settings.APP_INPUT_CHARSET, sign=settings.APP_SIGN_TYPE):
        self.APP_KEY = key
        self.APP_INPUT_CHARSET = charset
        self.APP_SIGN_TYPE = sign
    ...
    def params_filter(self, params, is_build=True):
        ...
        for k in ks:
            v = params[k]
            k = self.smart_str(k, self.APP_INPUT_CHARSET)
            if k not in ('sign', 'sign_type') and v != '':
                newparams[k] = self.smart_str(v, self.APP_INPUT_CHARSET)
                prestr += '%s=%s&' % (k, newparams[k])
        prestr = prestr[:-1]
        return newparams, prestr

    # 生成签名结果,str
    def build_mysign_str(self, prestr, sign_type='MD5'):
        if sign_type == 'MD5':
            return md5(prestr + self.APP_KEY).hexdigest()
        return ''
    ...
    def notify_verify(self, post):
        ...
        mysign = self.build_mysign_str(prestr, self.APP_SIGN_TYPE)
    ...
    def expires_verify(expires):
        if expires:
            ...
            return True if int(expires) > curr_time else False
        else:
            return False
